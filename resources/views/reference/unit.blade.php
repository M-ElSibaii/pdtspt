<x-app-layout>
    <div style="background-color: white;">
        <div class="container sm:max-w-full py-9">
            ...
            <h3 class="py-2">{{ Lg::pick('Unidade', 'Unit') }} <span class="text-sm text-gray-500">(EN ISO 23387 · UnitType)</span></h3>
            ...
            <table id="tblprop" cellpadding="0" cellspacing="0">
                <tbody>
                    ...
                    <tr><th>URI</th><td>
                        <a href="{{ Uri::link('unit', $unit->code) }}" target="_blank">{{ \App\Services\UnitsReference::unitUri($unit->code) }}</a>
                    </td></tr>
                    <tr><th>{{ Lg::pick('Código', 'Code') }}</th><td>{{ $unit->code }}</td></tr>
                    <tr><th>{{ Lg::pick('Nome', 'Name') }}</th><td>{{ $unit->name }}</td></tr>
                    <tr><th>{{ Lg::pick('Grandeza física', 'Physical quantity') }}</th><td>
                        @if ($quantity)
                            <a href="{{ Uri::link('pq', $quantity->name) }}">{{ $quantity->name }}</a>
